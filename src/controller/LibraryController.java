...
public class LibraryController implements HttpHandler {
    ...
    /** Book list entry — includes returnDate as null (returned on borrow response) */
    private String bookToJson(Book b) {
        return String.format(
            "{\"bookId\":%d,\"title\":\"%s\",\"author\":\"%s\",\"status\":\"%s\",\"returnDate\":null}",
            b.getBookId(), escape(b.getTitle()), escape(b.getAuthor()), b.getStatus().name()
        );
    }
    ...
    private String myBorrowingToJson(Borrowing b) {
        ...
        try {
            ...
            if (book != null) title = escape(book.getTitle());
        } catch (Exception ignored) {}

        return String.format(
            "{\"borrowingId\":%d,\"bookId\":%d,\"title\":\"%s\"," +
            "\"borrowDate\":\"%s\",\"returnDate\":\"%s\",\"returned\":%b}",
            b.getBorrowingId(), b.getBookId(), title,
