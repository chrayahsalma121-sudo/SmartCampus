...
public class RoomController implements HttpHandler {
    ...
    // =========================================================================
    private void handleListAllReservations(HttpExchange exchange) throws IOException {
        try {
            AuthenticatedUser authUser = AuthFilter.getAuthenticatedUser(exchange);
            if (authUser == null) {
                ResponseUtil.sendError(exchange, 401, "Unauthorized. Token missing or invalid.");
            ...
            ResponseUtil.sendSuccess(exchange, 200, "All reservations retrieved successfully.", sb.toString());
    ...
    private String reservationToJson(Reservation r) {
        String roomName = r.getRoomName() != null
            ? "\"" + escape(r.getRoomName()) + "\""
            : "null";
        String studentName = r.getStudentName() != null
            ? "\"" + escape(r.getStudentName()) + "\""
            : "null";

        return String.format(
            "{\"reservationId\":%d,\"studentId\":%d,\"studentName\":%s,\"roomId\":%d,\"roomName\":%s," +
            "\"reservationDate\":\"%s\",\"startTime\":\"%s\",\"endTime\":\"%s\",\"status\":\"%s\"}",
            r.getReservationId(), r.getStudentId(), studentName, r.getRoomId(), roomName,
